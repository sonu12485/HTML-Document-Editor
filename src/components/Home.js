    ...
    if (flag) {
      ...
      return (
        <Button
          ...
          onClick={() => this.props.deleteTemplate(name)}
        >
          DELETE
        </Button>
      );
        ...
        return (
          <div>
            <div
              className="new"
              onClick={() => {
                this.props.assign_template(JSON.parse(template.data));
                this.props.assign_template_body(JSON.parse(template.body));
                this.props.history.push("/new");
              }}
            >
              <div style={{ color: "#345F90", margin: 5, fontSize: 24 }}>
                {template.name}
              </div>
            </div>
  ...
  render() {
    return (
      <div>
        ...
        <div>
          <div className="home_text">Create a new HTML Document -</div>
        </div>
        ...
        <div style={{ paddingLeft: 50 }}>
          ...
          >
            <FA.FaPlus size={50} color="#345F90" style={{ margin: 5 }} />
          </div>
        </div>

        <div>
          <div className="home_text">Saved Templates -</div>
        </div>

        <div className="templates_container">{this.renderTemplates()}</div>
        <br />
        <br />
      </div>
    );
